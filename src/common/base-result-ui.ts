import { ImageKeys } from "../ellipse/config";
...
export class BaseResultUI<M extends Phaser.Scene> {
  ...
  init() {
    ...
    const restart = this.mainScene.add.image(0, 0, ImageKeys.RESTART_BUTTON);
    const home = this.mainScene.add.image(0, 0, ImageKeys.HOME_BUTTON);
